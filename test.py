  ...
  def test(self,config):
    ...
    try:
      while not self.coord.should_stop():
          ...
          ###########
          counter += 1
          samples, d_loss, g_loss = self.sess.run([self.multisamples, self.d_loss, self.g_loss],
                                                  feed_dict={self.z: sample_z,
                                                  self.zs: sample_multi_z})
          for i,sample in enumerate(samples):
            for times in xrange(self.output_frames+1):
              i_sample = sample[:,times,:,:,:]
              save_images(i_sample, [4, 4],
                      './test/test_{:02d}_{:02d}_{:02d}.png'.format(counter, i,times))
          
          print("[Sample] [z %.2d]d_loss: %.8f, g_loss: %.8f" % (counter, d_loss, g_loss))
